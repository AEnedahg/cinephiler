...
import { popularQueryFunc } from "@/_lib/query/query";
import { useRouter } from "next/navigation";
import Link from "next/link";

function PopularMovies() {
  const router = useRouter();
  const { data, isLoading, error } = popularQueryFunc();
  ...
  if (error)
    return <p className="text-red-500">Failed to load popular movies.</p>;
